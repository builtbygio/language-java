  ...
  const getTokens = function(buffer, row) {
    ...
    while (true) {
      ...
      if (end.column < line.length) {
        for (var num of Array.from(iterator.getCloseScopeIds())) {
          var item = scopes.pop();
        }
        scopes.push(...Array.from(iterator.getOpenScopeIds() || []));
        start = end;
    ...
    return (() => {
      ...
      for (let i = 0; i < tokens.length; i++) {
        ...
        for (var j = 0; j < token.length; j++) {
          var t = token[j];
          var scopes = (Array.from(t.scopes).map((scope) => `'${scope}'`)).join(", ");
...
`
  ...
  it('tokenizes reserved keywords', function() {
    let tokens = tokenizeLine('const value;');

    expect(tokens[0]).toEqual({value: 'const', scopes: ['source.java', 'keyword.reserved']});

    tokens = tokenizeLine('int a = 1; goto;');

    return expect(tokens[7]).toEqual({value: 'goto', scopes: ['source.java', 'keyword.reserved']});
});
...
`
  ...
  it('tokenizes identifiers with `$`', function() {
    const tokens = tokenizeLines(`\
class A$B {
  void func$() {
    $object.$property;
...
`
    ...
    return expect(tokens[6][1]).toEqual({value: 'Arrays', scopes: ['source.java', 'meta.class.body', 'meta.method', 'meta.method.body', 'storage.type']});
});
});
